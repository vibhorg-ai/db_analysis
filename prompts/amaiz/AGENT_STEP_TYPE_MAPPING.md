# Agent → Step mapping (v7)

This file maps each flow step to the **step type** you select in the AMAIZ UI when creating a new step.

---

## v7 mapping (2 flows, 2 steps total)

### Flow: `chat`

| Order | Step name | Step type | Behaviour | Purpose |
|-------|-----------|-----------|-----------|---------|
| 1 | `chat_response` | **LLM generation** | **Respond to user** | Interactive chat. v7 sends system prompt + chat history + user message as a single `user_input`. AMAIZ generates and returns the response. |

### Flow: `db_analysis_amaiz_pipeline`

| Order | Step name | Step type | Behaviour | Purpose |
|-------|-----------|-----------|-----------|---------|
| 1 | `pipeline_agent` | **LLM generation** | **Auto continue** | Generic LLM endpoint for all pipeline stages. v7 Python backend calls this flow once per agent stage, sending the full agent prompt + context as `user_input`. |
...
| v4 step | v7 equivalent |
|---------|---------------|
| security_agent | **Removed** — not in v7 |
...
| entity_graph | **Removed** — not in v7 main pipeline |
...
| query_plan_graph | **Removed** — not in v7 |
| optimizer_reasoning | Merged into `optimizer` in v7 |
| query_rewrite | Merged into `optimizer` in v7 |
| runtime_verification | **Removed** — not in v7 |
| index_advisor | Handled by `pipeline_agent` (called with index_advisor prompt) |
| impact_analysis | **Removed** — not in v7 main pipeline |
...
**Key insight:** In v7, all pipeline stages go through the same single `pipeline_agent` AMAIZ step. The differentiation happens in the Python backend, which sends different prompts for each stage.
...
| **Code** | Not used in v7 AMAIZ setup. |
...
> **Note:** JSON Structured Output is **not required** in v7. Agents return free-form markdown; the Python backend processes raw text responses.

---

## Step type reference

Only **LLM generation** is used in v7. Available step types in the AMAIZ UI for reference:
