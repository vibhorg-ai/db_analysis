# db_analysis — AMAIZ Setup Guide (v7)

This guide covers the complete AMAIZ portal configuration for v7. The v7 architecture is fundamentally different from v4: **Python orchestrates the pipeline**, and AMAIZ provides **single-step LLM generation** per call.
...
| `security_agent` step      | In AMAIZ                                 | **Removed** — not in v7 pipeline          |
...
The chat flow should return an object with a `message` string (see schema below). The v7 backend accepts both a plain string and `{"message": "..."}` from AMAIZ and normalizes to a string for the API response.
...
v7 sends **context** (schema, health, reports, connection, rules) in `{{context_data}}` and **conversation** (user/assistant turns) in `{{user_input}}`. Use **both** so the model sees the database context:
...
> **Important:** v7 manages chat history in the Python backend (`ChatSession`). **Uncheck** "Include chat history" so messages are not duplicated.
...
> **Note:** JSON Structured Output is no longer required — v7 agents return free-form markdown, not structured JSON. The Python backend parses the raw response.
...
| **Configuration**    | No app_config needed for v7.                                                                                                                               |
| **Evaluation**       | Optional.                                                                                                                                                  |
...
If chat responses repeat prior messages, verify that **Include chat history** is **unchecked** in the chat_response step's Prompt tab. v7 manages its own chat history and embeds it in `user_input`.
...
The v7 backend also has an automatic fallback: if the pipeline flow fails, it retries with the `chatbot` flow so analysis still works while you fix the pipeline flow.
...
v7 expects a raw text response from AMAIZ. If the `PipelineAgentResponse` schema causes issues, try removing the Response JSON schema entirely from the `pipeline_agent` step — the Python backend doesn't rely on structured output.
