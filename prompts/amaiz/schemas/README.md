...
In v5, all agents return **free-form markdown** in a single `message` field. The Python backend receives the raw text and processes it. This simplification was made because:
...
The Response JSON schema is **optional** in v5. If you encounter issues with the LLM not returning valid JSON matching the schema, you can remove the Response JSON schema entirely. The Python backend does not rely on structured JSON output from AMAIZ — it reads the raw `response.message.message` text.
