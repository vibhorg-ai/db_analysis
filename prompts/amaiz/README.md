# AMAIZ prompts — v5 (GenAI app flow)

This folder contains prompts and setup instructions for the **db_analysis** GenAI app in v5.
...
In **v5**, the Python backend (`AgentOrchestrator`) orchestrates the pipeline. It calls AMAIZ **separately for each agent stage**, sending the full prompt + context each time. AMAIZ now acts as a **single-step LLM endpoint** — it receives a prompt, runs one LLM generation, and returns the response.
...
- No `security_agent` step — removed from v5 pipeline.
...
## v5 pipeline stages (orchestrated in Python)
