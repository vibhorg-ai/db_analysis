"""
FastAPI application for DB Analyzer AI v7.
...
def create_app() -> FastAPI:
    ...
    # Middleware order: last added = outermost (first to receive request)
    # Request flow: CORS -> RateLimit -> BodySizeLimit -> APIKey -> RequestLogging -> routes
    ...
    # Allowed CORS origins: from settings plus optional extra origins (set per environment)
    _origins_from_settings = (
        ["*"]
        if settings.cors_allowed_origins == "*"
        else [o.strip() for o in settings.cors_allowed_origins.split(",") if o.strip()]
    )
    _extra_origins = [o.strip() for o in (settings.cors_extra_origins or "").split(",") if o.strip()]
    _cors_origins = (
        _origins_from_settings
        if _origins_from_settings == ["*"]
        else list(dict.fromkeys([*_origins_from_settings, *_extra_origins]))
    )
    app.add_middleware(
        CORSMiddleware,
    ...
    async def health() -> dict:
        ...
        snapshot = get_active_connections_snapshot()
        db_status: dict[str, Any] = {}
        for conn_id, conn in snapshot.items():
            engine = conn.get("engine", "unknown")
            label = conn.get("name") or conn_id
            connector = conn.get("connector")
            connected = connector is not None and getattr(connector, "_pool", None) is not None
            db_status[conn_id] = {
                "engine": engine,
                "label": label,
                "connected": connected,
