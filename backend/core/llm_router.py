"""
LLM router v7: AMAIZ-only. Session init and payload via AmaizService.
...
class PipelineRetriesExhaustedError(RuntimeError):
    """Raised when all retry attempts for a transient error have been exhausted."""
    pass
...
class LLMRouter:
    ...
    If the pipeline flow fails (e.g. "last step" error), all subsequent
    LLMRouter instances skip it and use the chat flow directly, avoiding
    minutes of wasted time per agent.
    """

    _pipeline_flow_broken: bool = False
    ...
    async def _generate_with_flow(
        ...
        self._session_id = None
        raise PipelineRetriesExhaustedError(
            f"All {MAX_RETRIES} retries exhausted: {last_err}"
        ) from last_err
    ...
    async def generate(
        ...
        **kwargs: Any,
        ...
        if not is_chat and getattr(self._settings, "USE_CHAT_FLOW_FOR_PIPELINE", False):
            logger.info(
                "USE_CHAT_FLOW_FOR_PIPELINE=true — using chat flow '%s' for pipeline request",
                chat_flow,
            )
            return await self._generate_with_flow(instruction, context_str, chat_flow)

        if not is_chat and self._pipeline_flow_broken:
            logger.info(
                "Pipeline flow '%s' previously failed — using chat flow '%s' directly",
                resolved_flow,
                chat_flow,
            )
            return await self._generate_with_flow(instruction, context_str, chat_flow)
        ...
        try:
            ...
            raise
        except PipelineRetriesExhaustedError:
            raise
        except Exception as primary_err:
            if is_chat:
                raise

            self._pipeline_flow_broken = True
            logger.warning(
                "Pipeline flow '%s' failed (%s). Marking broken; falling back to chat flow '%s'.",
                resolved_flow,
                primary_err,
                chat_flow,
            )
