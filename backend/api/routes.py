...
async def analyze_query(
    ...
    """
    logger.info(
        "analyze-query received: mode=%s, query_len=%d, connection_id=%s",
        req.mode,
        len(req.query or ""),
        req.connection_id or "none",
    )
    try:
            ...
            if engine == "postgres" and connector:
                ...
                if tables is None:
                    try:
                        ...
                        logger.warning("Could not fetch schema for analysis: %s", schema_err, exc_info=True)
                        tables = []
        ...
        # Option B: no active connection → downgrade "full" to "fast"
        if mode == "full" and not conn:
            logger.info("No active connection — downgrading mode from 'full' to 'fast'")
            mode = "fast"

        if mode == "full":
            result = await orchestrator.run_full(context)
        elif mode == "fast":
            result = await orchestrator.run_fast(context)
